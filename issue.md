...
`Init(config map[string]interface{}) error` вот тут вообще наверное контекст должен быть, то есть что за проект какой у него сторейдж и тд...

Так же в `pkg` нужно перенести некие хелперы для разработки плагинов, потому что телеграм вышел портяночный...

Например в плагине как то юзать `projectCtx.storage.Path(...), projectCtx.getContext(), projectCtx. ... и тд`
