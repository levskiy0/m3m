...
runtime api - шаблонизатор

----

Yaml шаблоны проектов
Хранятся на беке, можно текущи проект экспортировать, сохранить как шаблон, импортировать.

При создании проекта можно выбрать шаблон
...
Плагины тоже могут отдавать шаблоны их применения.

На фронте мы при создании проекта можем выбирать шаблон из списка (желательно еще бить по категориям как то...)
...
``` 
- Настройка вида таблицы (выбор полей, выбор по каким мы будем фильтровать, сортировать, делать поиск и тд)
- Настройка формы (отображаются поля, можно скрыть те которые по умолчанию и менять их местами), настройка представления, например `Text` может иметь вид или `Textarea` или `TipTap`. Настройка валидации.
```
